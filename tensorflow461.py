...
'''###################################################################################################
OLD ATTEMPT - keeping just in case

# Setting the target for what we would like to predict
# I believe this syntax results in if the tweet was republican it is 1, vice versa
twitter['target'] = np.where(twitter['party_id'] == 'R', 0, 1)

# Creating different data sets for multiple purposes
train, validate, test = np.split(twitter.sample(frac = 1), [int(.8*len(twitter)), int(.9*len(twitter))])

#print(len(train), 'training')
#print(twitter.head)

# Converts the three pandas datasets created into a tensorflow dataset
def dataConvert(twitter, shuffle = True, batch_size=32):
    twitt = twitter.copy()
    labels = twitt.pop('target')
    twitt = {key: value[:, tf.newaxis] for key, value in twitter.items()}
    twittSet = tf.data.Dataset.from_tensor_slices((dict(twitter), labels))
    if shuffle:
        twittSet = twittSet.shuffle(buffer_size = len(twitter))
    twittSet = twittSet.batch(batch_size)
    twittSet = twittSet.prefetch(batch_size)
    return twittSet
                                    
batch_size = 5
trainTwitter = dataConvert(train, batch_size = batch_size)
[(train_features, label_batch)] = trainTwitter.take(1)
...
def normalize(name, data):
    ...
    dataFeature = data.map(lambda x, y: x[name])
    normal.adapt(dataFeature)

    return normalize

retweetCount = train_features['retweet_count']
layer = normalize('retweet_count', trainTwitter)
layer(retweetCount)
